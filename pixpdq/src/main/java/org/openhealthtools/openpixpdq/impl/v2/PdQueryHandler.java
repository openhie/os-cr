...
class PdQueryHandler extends BaseHandler implements Application {
   ...
   private Message processQCN_J01(QCN_J01 qcnMsg) throws DataTypeException, PixPdqException {
	 ...
	 * </ul>
	 * 
	 * @param reply the RSP_K21 reply message
	 * @param hl7Header the HL7Header
	 * @param queryTag the query tag String
	 * @throws HL7Exception if an HL7 problem occurs
	 * @throws PixPdqException if a PIX/PDQ problem occurs
	 */
    ...
    * @throws DataTypeException if a problem occurs
    */
    ...
    * @param isContinuation <code>true</code> - if continuation query is handled by OpenPIXPDQ; 
...
//            }
    ...
    */
    private int populatePID(PID pid, Patient patient, int setId, List<Identifier> returnDomains) throws HL7Exception, IheConfigurationException, PatientException {
        ...
        final List<PatientIdentifier> pids = patient.getPatientIds();
        ...
        for (int i = 0; i < pids.size(); i++) {
            final PatientIdentifier id = pids.get(i);
        ...
        populateXPN(pid.getPatientName(0), patient.getPatientName());
        
        // PID-6 - Mother's maiden name, we have only last name
        if (null != patient.getMothersMaidenName()) {
            populateXPN(pid.getMotherSMaidenName(0), patient.getMothersMaidenName());
        }
        
        // PID-7 - Birth date
        populateTS(pid.getDateTimeOfBirth(), patient.getBirthDateTime());
        
        // PID-8 - Gender
        final SexType sex = patient.getAdministrativeSex();
        final String gender;
        if (sex == SharedEnums.SexType.MALE) {
            gender = "M";
        } else if (sex == SharedEnums.SexType.FEMALE) {
            gender = "F";
        } else if (sex == SharedEnums.SexType.OTHER) {
            gender = "O";
        } else {
            gender = "U";
        }
        ...
        if (patient.getPatientAliases() != null) {
            for (final PersonName alias : patient.getPatientAliases()){
        ...
        // PID-11 - Addresses
        i = 0;  
        final List<Address> addresses = patient.getAddresses();
        if (addresses != null) {
        ...
        i = 0; 
        int j = 0;
        final List<PhoneNumber> phones = patient.getPhoneNumbers();
        if (phones != null) {
	        for (final PhoneNumber phone : phones) {
	            final PhoneType type = phone.getType();
	            // PID-14 - Work phone
	            if (type == SharedEnums.PhoneType.WORK ||
	        		type == SharedEnums.PhoneType.SERVICE ||
	        		type == SharedEnums.PhoneType.FAX) { 
	        		populateXTN(pid.getPhoneNumberBusiness(j++), phone);
	        	// PID-13 - Home phone
	        	} else if (i == 0 || type == SharedEnums.PhoneType.HOME ||
        ...
        populateCE(pid.getPrimaryLanguage(), patient.getPrimaryLanguage());
        ...
        final PatientIdentifier accountNumber = patient.getPatientAccountNumber();
        ...
        if (patient.getDriversLicense() != null) {
            final DriversLicense license = patient.getDriversLicense();
            ...
            if (license.getExpirationDate() != null) {
                final Calendar expDate = license.getExpirationDate();
                final int year = expDate.get(Calendar.YEAR);
                final int month = expDate.get(Calendar.MONTH) + 1;
                final int date = expDate.get(Calendar.DATE);
        ...
        populateTS(pid.getPatientDeathDateAndTime(), patient.getDeathDate());
    ...
    private final void populateTS(final TS ts, final Calendar cal) throws DataTypeException, IheConfigurationException {
        if (cal != null) {
            final String formatFromProperty = Configuration.getPropertySetValue(connection, "DateTimeFormat", "Birthdate", false);
            final String birthdateFormat = StringUtil.goodString(formatFromProperty) ? formatFromProperty : HL7v25.birhtdateFormat;
            final String date = new SimpleDateFormat(birthdateFormat).format(cal.getTime());
            ts.getTime().setValue(date);
        }
    }

     /**
     * Populates a CE component with an ID
     *
     * @param ce the CE component to populate
