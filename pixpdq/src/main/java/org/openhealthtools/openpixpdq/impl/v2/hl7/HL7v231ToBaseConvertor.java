...
public class HL7v231ToBaseConvertor{
    ...
    public List<NextOfKin> getNextOfKin() {
        List<NextOfKin> ret = new ArrayList<NextOfKin>();
        if (nk1Array != null) {
            for (NK1 nk1 : nk1Array) {
                SI setId = nk1.getSetIDNK1();
                if (setId != null && isValued(setId.getValue())) {
    ...
    public String getPrimaryLanguage(){
        final CE ce = pid.getPrimaryLanguage();
        final String text = ce.getText().getValue();
        return isValued(text) ? text : ce.getIdentifier().getValue();
    }
    ...
    public int getBirthOrder(){
    	...
    	return Integer.parseInt(pid.getBirthOrder().getValue());
    ...
    private PhoneNumber _getPhoneNumber(XTN xtn) {
        ...
        if(xtn != null) {
            final String sNum = xtn.get9999999X99999CAnyText().getValue();
            final String area = xtn.getAreaCityCode().getValue();
            final String local = xtn.getPhoneNumber().getValue();
            final String email =  xtn.getEmailAddress().getValue();
            if(isValued(sNum) || isValued(area) || isValued(local)) {
                number = new PhoneNumber();
                number.setRawValue(sNum);
                number.setAreaCode(area);
                number.setExtension(xtn.getExtension().getValue());
                number.setNumber(local);
            }
            if(isValued(email)) {
                if (number == null) {
                    number = new PhoneNumber();
    ...
    private final static boolean isValued(final String s) {
